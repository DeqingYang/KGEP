...
lines = open('../data/book/original/train.txt', 'r').readlines() + open('../data/book/original/test.txt', 'r').readlines()
...
kg_np = np.loadtxt('../data/book/original/kg_final.txt', dtype=np.int32)
...
for line in kg_np:
    ...
    graph_map[line[2][0]][line[2]].append([int(line[1])+39, line[0]])
...
for u_id in user_histroy:
    ...
    for pos in user_histroy[u_id]:
        ...
        if len(graph_map['u']['u' + str(u_id)])<partition:
            graph_map['u']['u' + str(u_id)].append([78,'i' + str(pos)])
            graph_map['i']['i' + str(pos)].append([79, 'u' + str(u_id)])
    Interact_tuple_train += tmp[: partition]
    Interact_tuple_test += tmp[partition:]


with open('../data/book/graph_map.json', 'w') as f:
    f.write(json.dumps(graph_map))


save_pickle([Interact_tuple_train], '../data/book/Interact_tuple_train.dat')
...
test_set_book=collections.defaultdict(dict)#只包含5个正样本和45个负样本
for user in test_new:
    if len(test_new[user]['pos'])==5 and len(test_new[user]['neg'])==45:
        test_set_book[user]=test_new[user]
    
save_pickle([test_set_book], '../data/book/test_set.dat')
print('done')